...
__global__ void CUDA_CheckEdge(unsigned char* sobel, int width, int height, int min_val, int max_val, int i, int j, unsigned int* stack, unsigned int* top, unsigned char* output, unsigned char*visited);
...
void CUDA_Canny()
{
	...
	while (1)
	{
		...
		cudaMemcpy(temp.data, gpu_sobel, width * height * sizeof(unsigned char), cudaMemcpyDeviceToHost);
		imshow("temp", temp);


		/*4.double threshold*/
		//CUDA_DoubleThreshold << <grid_size2, block_size_normal2 >> > (gpu_sobel, width, height, 40, 90, gpu_canny);
		CUDA_DoubleThreshold2 << <grid_size2, dim3(1,1) >> > (gpu_sobel, width, height, 40, 60, gpu_canny);
		...
		for (int j = 0; j < width; j++)
		{
			if (IS_STRONG_EDGE(CUDA_GetPixelVal(sobel, width, height, i, j)))
			{
				...
				while ((*stack_top) > 0)
				{
					...
					CUDA_IsWeakEdge(sobel, width, height, min_val, max_val, center_index / width + 1, center_index % width    , weak_stack, stack_top, output, visited);
					CUDA_IsWeakEdge(sobel, width, height, min_val, max_val, center_index / width + 1, center_index % width + 1, weak_stack, stack_top, output, visited);
					//CUDA_CheckEdge << <1, 1 >> > (sobel, width, height, min_val, max_val, center_index / width, center_index % width, weak_stack, stack_top, output, visited);
